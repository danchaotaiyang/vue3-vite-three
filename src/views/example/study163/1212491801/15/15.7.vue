...
rgbELoader.setPath('/static/example/study163/1212491801/texture/');

let beach = await rgbELoader.loadAsync('env/hdr/Beach.hdr');
beach.mapping = THREE.EquirectangularReflectionMapping;
scene.background = beach;
scene.environment = beach;
...
const water = new Water(geometryPlane, {
    color: 0xffffff,
    scale: 1,
    flowDirection: new THREE.Vector2(1, 1),
    textureWidth: 1024,
    textureHeight: 1024,
    normalMap0: textureLoader.load('water/Water_1_M_Normal.jpg'),
    normalMap1: textureLoader.load('water/Water_2_M_Normal.jpg')
});
water.rotation.x = -Math.PI / 2;
scene.add(water);
*/

gltfLoader.load('pool/pool.glb', (glb) => {
    const meshPool = glb.scene.getObjectByName('pool');
    console.log(meshPool);
    if (meshPool[ 'isMesh' ]) {
        meshPool[ 'material' ].side = THREE.DoubleSide;
        scene.add(meshPool);
    }
    const meshWater = glb.scene.getObjectByName('water');
    if (meshWater[ 'isMesh' ]) {
        // meshWater[ 'material' ].side = THREE.DoubleSide;
        const water = new Water(meshWater[ 'geometry' ], {
            color: 0xffffff,
            scale: 1,
            flowDirection: new THREE.Vector2(1, 1),
            textureWidth: 1024,
