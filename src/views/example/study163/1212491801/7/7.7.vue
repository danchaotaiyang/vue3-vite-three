...
folderControls.add(state, 'translationSnap', 0, 5, .1).name('移动步长').onChange((value) => {
    if (value === 0) {
...
folderControls.add(state, 'scaleSnap', 0, 5, .1).name('缩放步长').onChange((value) => {
    if (value === 0) {
...
folderControls.add(state, 'rotationSnap', 0, Math.PI, .1).name('旋转步长').onChange((value) => {
    if (value === 0) {
        tControls.setRotationSnap(null);
    } else {
        tControls.setRotationSnap(value);
    }
});
folderControls.add(state, 'clamp', null).name('吸附地面').onChange((value) => {
    tControls.setSpace(value ? 'local' : 'world');
});
folderControls.add(state, 'spaceLocal', null).name('本地坐标').onChange((value) => {
    tControls.setSpace(value ? 'local' : 'world');
});
...
const folderEnv = gui.addFolder('环境');
folderEnv.add(state, 'light', null).name('关灯').onChange((value) => {
    renderer.toneMappingExposure = value ? .1 : 1;
});
