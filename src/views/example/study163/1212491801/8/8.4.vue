...
//  创建动画剪辑
let clip = new THREE.AnimationClip('move', 4, [ keyframePosition, keyframeRotation, keyframeBoolean ]);
//  创建动画动作
...
gltfLoader.load('moon.glb', (glb) => {
    scene.add(glb.scene);
    let keyframeMoon = new THREE.BooleanKeyframeTrack(
        'defaultMaterial.visible',
        [ 0, 1, 2, 3, 4 ],
        [ true, false, true, false, true ]
    );
    mixerMoon = new THREE.AnimationMixer(glb.scene);
    let clipMoon = new THREE.AnimationClip('bool', 4, [ keyframeMoon ]);
