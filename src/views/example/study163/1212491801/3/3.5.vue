...
const setBoundingBox = (glb) => {
    ...
    */
    ...
    ══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
    */
    let boxDuckCenter = boxDuck.getCenter(new THREE.Vector3());
    console.log(boxDuckCenter);
    //  更新加载物体的世界矩阵
    meshDuck.updateWorldMatrix(true, true);
    //  包围盒应用世界矩阵
    ...
    //  包围球
    geometryDuck.computeBoundingSphere();
    let sphereDuck = geometryDuck.boundingSphere;
    sphereDuck.applyMatrix4(meshDuck.matrixWorld);
    let geometryHelperSphere = new THREE.SphereGeometry(sphereDuck.radius, 16, 16);
    let materialHelperSphere = new THREE.MeshBasicMaterial({
        color: 0xff00ff,
        wireframe: true
    });
    let meshHelperSphere = new THREE.Mesh(geometryHelperSphere, materialHelperSphere);
    meshHelperSphere.position.copy(sphereDuck.center);
    scene.add(meshHelperSphere);
};
