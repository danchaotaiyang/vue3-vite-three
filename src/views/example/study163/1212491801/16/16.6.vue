...
import baseFragmentShader from './shader/16.6/fragment.glsl?raw';
import baseVertexShader from './shader/16.6/vertex.glsl?raw';
...
const createPoints = (option = {}) => {
    ...
    let points = new THREE.Points(geometry, materialShader);
    scene.add(points);
    return { geometry, materialShader, points };
};
...
const update = () => {
    materialShader.uniforms.uTime.value = clock.getElapsedTime();
};
