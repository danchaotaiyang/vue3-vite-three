...
objectLoader.setPath(`${ base }/static/example/study163/1212491801/`);
...
/*gltfLoader.load('/diamond/diamond.glb', (glb) => {
    let meshDiamond = glb.scene.getObjectByName('blast1_diamond_0');
    if (meshDiamond && meshDiamond[ 'isMesh' ]) {
        meshDiamond[ 'material' ].flatShading = true;
    }
    scene.add(glb.scene);
});*/

objectLoader.load(`models/diamond/diamond.json`, (object) => {
    scene.add(object);
});
