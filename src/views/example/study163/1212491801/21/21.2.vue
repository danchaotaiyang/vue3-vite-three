...
const pointsCurve = curve.getPoints(500);
...
const meshCurve = new THREE.Line(geometryCurve, materialCurve);
...
const update = () => {
    ...
    let time = (elapsedTime / 10) % 1;
    let point = curve.getPoint(time);

    camera.lookAt(meshEarth.position);
    camera.position.copy(point);

    meshMoon.position.copy(point);
